...
from ..utilities.exceptions import BadRequest, InvalidRole
...
class BaseGroup(BaseItem):
    ...
    async def get_member_rank(self, user: Union[int, BaseUser]) -> MemberRelationship:
        """
        Gets a members group rank.
        ...
        user = await self._shared.client.get_user(user)
        rank_response = await self._shared.requests.get(
            url=self._shared.url_generator.get_url("groups", f"v2/users/{user.id}/groups/roles")
        )
        role_data = rank_response.json()
        for group in role_data['data']:
